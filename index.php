<?php
    ...
    // $photo = $row1['photo_doc'];
    // $_SESSION['photo'] = $photo;

    $sql = "SELECT * FROM u_music";
    $result = mysqli_query($con,$sql);

    if(isset($_GET['c_btn']))
    {
        $ty_con = $_GET['type_con'];
    ...
    {
        $play_music = $_GET['playMusic'];

        $sql2 = "SELECT * FROM u_music WHERE id='$play_music'";
        $result2 = mysqli_query($con,$sql2);
        $row2 =mysqli_fetch_assoc($result2);

        $song_name = $row2['song'];
        $artist_name = $row2['u_name'];
        $title = $row2['title'];
        $song_id = $row2['id'];
...
<body>
    ...
    <div class="content">
        ...
        <div class="search-1">
            <input type="text" placeholder="Type here" name="search_box" value="<?php if(isset($_GET['search_box'])){echo $_GET['search_box'];}?>">
        ...
        <video autoplay loop muted plays-inline class="bgvideo-1">
            <source src="Image/Music2.mp4">
        </video>
        <div class="music">
            <div class="m1">
                <table>
                <?php
                    if(isset($_GET['srch_btn']))
                    {
                        $cnt=1;
                    }
                    if($cnt==0)
                ...
                ?>    
                        <tr>
                            <td class="td1"><img src="musicCoverUploads/<?php echo $row['cover_photo']; ?>"></td>
                            <td class="td2">
                                <span>
                                    <?php echo $row['title'];?>
                                    <br>
                            ...
                            <td class="td3"><button name="playMusic" value="<?php echo $row['id'];?>"><ion-icon class="playMusic" name="play" id="songPlay"></button></td>
                            <td class="td4"><span>show more</span></td>
                        </tr>
                    <?php
                        }
                    }
                    ...
                    if(isset($_GET['search_box']))
                    {
                        if($cnt==1)
                        {
                            ...
                            $sql4 = "SELECT * FROM u_music WHERE CONCAT(u_name,title) LIKE '%$search_value%'";
                            $result4 = mysqli_query($con,$sql4);

                            if(mysqli_num_rows($result4) > 0)
                            {
                                foreach($result4 as $songs)
                                {
                    ?>
                                    <tr>
                                        <td class="td1"><img src="Image/Profile.jpg"></td>
                                        <td class="td2">
                                            <span>
                                                <?php echo $songs['title'];?>
                                                <br>
                                                <div class="a_name">
                                                    <?php echo $songs['u_name'];?>
                                                </div>
                                            </span>
                                        </td>
                                        <td class="td3"><button name="playMusic" value="<?php echo $row['id'];?>"><ion-icon class="playMusic" name="play" id="songPlay"></button></td>
        ...
        <div class="music-box">
            <center>
                <input type="range" name="range" class="range" id="myProgressBar" max="100" min="0" value="0">
                <div class="u-m-box">
                    <div class="name-box">
                    ...
                    <div class="button-box">
                        <img src="Image/backward-step-solid.svg" id="prePlay">
                        <img src="Image/circle-play-regular.svg" id="masterPlay" class="playMusic">
                        <img src="Image/circle-pause-regular.svg" id="masterPause" class="pauseMusic">
                        <img src="Image/forward-step-solid.svg" id="nextPlay">
                    </div>
                </div>
            </center>
        </div>
...
<?php $songId = "<script>document.write(pSongId)</script>"?>

<?php

$sql2 = "SELECT * FROM u_music WHERE id='$songId'";
$result2 = mysqli_query($con,$sql2);
$row2 =mysqli_fetch_assoc($result2);
...
//     if(audioElement.paused || audioElement.currentTime<=0) {
//         audioElement.play();
//         songPlay.classList.remove("playMusic");
//         document.getElementById("songPause").style.display = "block";
//         songPlay.classList.add("pauseMusic");
//     }
// })
// songPause.addEventListener('click',()=>{
//     if(audioElement.played) {
//         audioElement.pause();
//         songPlay.classList.remove("pauseMusic");
//         document.getElementById("songPause").style.display = "none";
//         songPlay.classList.add("playMusic");
//     }
// })
